...
function Board() {
    ...
    const [winType, setWinType] = useState<'horizontal' | 'vertical' | 'diagonal' | 'diagonal-reverse' | null>(null);
    ...
    const calculateWinner = (squares: (string | null)[]) => {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (squares[a] && squares[a] === squares[b] && squares[a] === squares[c]) {
                let type: 'horizontal' | 'vertical' | 'diagonal' | 'diagonal-reverse';
                
                if (i < 3) type = 'horizontal';
                else if (i < 6) type = 'vertical';
                else if (i === 6) type = 'diagonal';
                else type = 'diagonal-reverse';
                
                return { winner: squares[a], line: [a, b, c], type };
            }
        }
        return null;
    };
    ...
    const handleClick = (index: number) => {
        ...
        // Check for winner after updating the square
        const result = calculateWinner(newSquare);
        if (result) {
            ...
            <div className="grid grid-cols-3 gap-2">
                {square.map((value, index) => (
                    <Square
                        ...
                        winType={winningLine?.includes(index) ? winType : undefined}
                    />
                ))}
            </div>
